...
function findOptionId(element){var re,id,optionId,updatedname,splitedName,name;if(!element){return id;}
updatedname=$(element).attr('name');if(updatedname.indexOf("[bundle_option]")>=0){splitedName=updatedname.split("[bundle_option]");optionId=splitedName[splitedName.length-1];name='bundle-option'+optionId;}else{name=$(element).attr('name');}
if(name.indexOf('[')!==-1){re=/\[([^\]]+)?\]/;}else{re=/_([^\]]+)?_/;}
